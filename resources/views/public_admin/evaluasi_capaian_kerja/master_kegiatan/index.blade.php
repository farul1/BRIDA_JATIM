        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-indigo">
                <div class="panel-heading">
                    <h6 class="panel-title">Data Master Kegiatan </h6>
                ...
                <div class="panel-body">
                    <div>
                        <a class="btn btn-info" onclick="tambah_kegiatan_evaluasi('{{csrf_token()}}', '#ModalBiruSm','{{$programEvaluasi->id}}')">Tambah Data
                    ...
                    <div class="">
                        <table class="table table-bordered table-hover datatable-basic">
                            ...
                            <tbody>
                                ...
                                $status='-';
                                ?>
                                @foreach ($kegiatan_evaluasi as $r)
                                @php
                                $no++; 
                                @endphp
                                ...
                                <tr>
                                    <td>{{$no}}</td>
                                    <td>{{$r->program->bidang->nama_bidang}}</td>
                                    <td>{{$r->program->uraian}}</td>
                                    <td>{{$r->uraian}}</td>
                                    <td align="center"> 
                                        <?php
                                        $key = saleh_encrypt($r->id . $r->created_at . '##' . $r->id . '##' . $r->id_bidang . $r->created_at, 'progstylysbysaleh');
                                        ?>
                                        <a href="{{ route('indikator_evaluasi',$key) }}" target="_blank" title="Indikator Evaluasi"
                                        class="btn btn-xs bg-green-800 btn-block">
                                            <i class="icon-archive"></i>  Indikator
                                        </a> 
                                    </td>
                                    <td>
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <button type="button" onclick="
                                                        ubah_kegiatan_evaluasi('{{csrf_token()}}', '{{ $r->id }}', '#ModalTealSm','{{$programEvaluasi->id}}')
                                                ...
                                                <li>
                                                    <button type="button"
                                                        onclick="hapus_kegiatan_evaluasi('{{csrf_token()}}','{{ $r->id }}')"
